...
//     const isAvailable = datePackages.some(pkg => {
//         // Force date parsing in UTC to prevent timezone shift
//         const pkgDate = dayjs(pkg.date).utc().format("YYYY-MM-DD");
//         return pkgDate === dayDate;
//     });
...
export default function Home() {
    ...
    const venueId = 2; // replace with actual venue ID
    ...
    const CustomDay = (props) => {
        ...
        const isAvailable = datePackages.some(pkg => pkg.date === formattedDay);
    ...
    const formatTime = (timeStr) => {
        const parsed = dayjs(timeStr); // removed utc/local confusion
        return parsed.isValid() ? parsed.format('h:mm A') : 'Invalid time';
    };
            ...
            <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
                ...
                <DialogContent>
                    <Typography>
                        ...
                        at <strong>{selectedSlot?.venueName || "Venue"}</strong>?
                    </Typography>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setOpenDialog(false)}>Cancel</Button>
